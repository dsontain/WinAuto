...
class ScreenPrintWin(object):
    ...
    @classmethod
    def save_bitmap(cls, bmp_filename='clipboard', window=True, tag=False, need_png=True):
        if tag:
        ...
        for cnt in range(0, 3):
            ...
            if data:
                break
            if cnt == 2:
                logging.info('Three attempts to capture failed')
            time.sleep(0.5)
...
def image_add_label(text="", imagefile="", output="",
                    position=(0, 0), rgb=(255, 0, 0), size=24, font=r"C:\Windows\Fonts\simhei.ttf"):
